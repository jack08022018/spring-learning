...
object PdfService {
    ...
    fun buildPart_A(document: Document, pdfCommonUtils: PdfCommonUtils) {
        val tdCss = ("border: 1px solid black;"
                + "text-align: left;"
                + "padding: 8px;")
        val tableCss = ("font: 14px verdana;"
        ...
        val html = (""
                ...
                + "</tr>"
                + "</table>")
        document.add(pdfCommonUtils.getElementFromHtml(html) as Table)

//        val url = "/src/main/resources/static/images/logo.png"
//        val imgHtml = """
//            <img style='width: 50px; height: 100px;border: 1px solid black;' src='${url}'/>
//        """.trimIndent()
//        document.add(pdfCommonUtils.getElementFromHtml(imgHtml) as Image)
        val imgHtml = """
		...
		"""
		document.add(pdfCommonUtils.getElementFromHtml(imgHtml) as Div);
    }
    ...
    fun buildCaution(document: Document, pdfCommonUtils: PdfCommonUtils) {
        val css = (""
                ...
                + "padding: 5px;")
        val html = ("<div style='" + css + "'>"
                + "<b>LƯU Ý:</b> Việc kê khai đầy đủ và trung thực các thông tin dưới đây sẽ quyết định tính hiệu lực của hợp đồng bảo hiểm và việc chi trả các quyền lợi bảo hiểm sau này của Quý khách." //					+ 	"<img style='width: 140px; height: auto;' src='src/main/resources/static/images/Prudential_1.png'/>"
                + "</div>")
        document.add(pdfCommonUtils.getElementFromHtml(html) as Div)
    }

    fun buildPart_B(document: Document, pdfCommonUtils: PdfCommonUtils) {
        val mainCss = (""
                + "font: 14px verdana;"
                + "background: white;"
                + "width: 100%;"
                + "line-height: 20px;"
                + "margin-top: 10px;")
        val numberingCss = "width: 20px; display: inline-block; float: left;"
        val html = (""
                + "<div style='" + mainCss + "'>"
                ...
                + "</b>"
                + "</td>"
                + "<td style='border: 1px solid black; padding: 8px;'>" //                +	    		"<div style='" + numberingCss+ "'>2.</div>"
                //                +	    		"<div style='display: inline-block;'>Giới tính:<br>"
                + "<b>Nam</b>" //                +    			"</div>"
                + "</td>"
                + "<td style='border: 1px solid black; padding: 8px;'>" //                +	    		"<div style='" + numberingCss+ "'>3.</div>"
                //                +	    		"<div style='display: inline-block;'>Số CMND/CCCD/Hộ chiếu:<br>"
                + "<b>766655544345</b>" //                +    			"</div>"
                + "</td>"
                + "</tr>" //                +	  	"<tr>"
                ...
                //                +	    	"</td>"
                //                +	  	"</tr>"
                + "</table>" //                +	"<div style='padding: 8px; border: 1px solid black; border-top: 0px; font-size: 13px;'>"
